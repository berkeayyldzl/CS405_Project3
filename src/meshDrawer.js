...
void main()
{
	...
	diff = max(dot(normal, lightdir), 0.0); //calculates the dot product of normal and direction of light, max ensures its positivity

	vec3 viewDir = normalize(-fragPos); //takes the fragment position and makes it according to camera
	vec3 reflectDir = reflect(-lightdir, normal);//to reflect light
	spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp);//the specular component
